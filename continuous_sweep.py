...
def run_sweep(
    ...
    pass_label = "∞" if continuous else str(passes)
    pass_idx   = 0
    try:
        while continuous or pass_idx < passes:
            print(f"\n[Pass {pass_idx+1}/{pass_label}]")

            for hop_idx, centre in enumerate(centres):
                seg_name  = (f"seg_{total_hops:04d}_"
                             f"{int(centre/1e6)}MHz")
                bin_path  = os.path.join(out_dir, seg_name + ".dat")
