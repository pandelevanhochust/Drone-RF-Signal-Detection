...
# ── Added for loss-curve plotting (headless-safe backend) ────────────────────
import numpy as np
import matplotlib
matplotlib.use("Agg")
...
def plot_training_loss(losses, out_path: str = "training_loss_curve.png") -> None:
    """
    Render the per-epoch training loss curve in the report format:
    real loss scatter + faint connecting line, EMA-smoothed trend (dashed),
    and a heteroscedastic ±1.5σ variance band that narrows as training converges.
    """
    losses = np.asarray(losses, dtype=float)
    n = len(losses)
    if n < 2:
        log.warning("Only %d epoch(s) recorded — skipping loss-curve plot.", n)
        return

    epochs = np.arange(1, n + 1)

    # ── Smoothed trend: exponential moving average over epochs ──────────────
    alpha = 0.35
    smooth = np.empty(n)
    smooth[0] = losses[0]
    for i in range(1, n):
        smooth[i] = alpha * losses[i] + (1.0 - alpha) * smooth[i - 1]

    # ── Variance band: rolling std of residuals (auto-shrinks on convergence)
    residual = losses - smooth
    win = 5
    band = np.empty(n)
    for i in range(n):
        lo = max(0, i - win // 2)
        hi = min(n, i + win // 2 + 1)
        band[i] = 1.5 * (np.std(residual[lo:hi]) + 1e-6)

    # ── Draw ────────────────────────────────────────────────────────────────
    plt.rcParams.update({"font.size": 12, "font.family": "DejaVu Sans"})
    fig, ax = plt.subplots(figsize=(9, 5.5), dpi=150)

    ax.fill_between(epochs, smooth - band, smooth + band,
                    color="#4a76a8", alpha=0.18, label="±1.5σ variance band")
    ax.plot(epochs, losses, color="#2f5d8a", lw=1.4, alpha=0.55, zorder=2)
    ax.scatter(epochs, losses, color="#2f5d8a", s=22, zorder=3, label="Training loss")
    ax.plot(epochs, smooth, color="#c1763a", lw=2.2, ls="--", zorder=4, label="Smoothed trend")

    # annotate first / last epoch loss
    ax.annotate(f"{losses[0]:.2f}", (epochs[0], losses[0]), textcoords="offset points",
                xytext=(8, 8), color="#2f5d8a", fontweight="bold")
    ax.annotate(f"{losses[-1]:.2f}", (epochs[-1], losses[-1]), textcoords="offset points",
                xytext=(-30, 10), color="#2f5d8a", fontweight="bold")

    ax.set_title(f"Training Loss — EfficientNet-B0 ({n} epochs)", fontsize=14, pad=12)
    ax.set_xlabel("Epoch")
    ax.set_ylabel("Loss (CrossEntropy + label smoothing)")
    ax.set_xlim(0.5, n + 0.5)
    y_top = float(max(losses.max(), (smooth + band).max())) * 1.1
    ...
    plt.close(fig)
    log.info("Training loss curve saved → '%s'", out_path)
...
def train(model: nn.Module, train_loader: DataLoader, val_loader: DataLoader, device: torch.device, num_classes: int = 3, num_epochs: int = 50, lr: float = 3e-4, weight_decay: float = 1e-2, checkpoint_path: str = "best_model.pth", early_stop_patience: int = 10, warmup_epochs: int = 5, class_names: list = None, loss_curve_path: str = "training_loss_curve.png") -> nn.Module:
    ...
    best_val_acc = 0.0
    no_improve_count = 0
    epoch_losses = []  # per-epoch training loss history (for loss curve)
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        epoch_losses.append(train_loss)  # record for loss curve
    ...
    # Render the training loss curve in report format
    plot_training_loss(epoch_losses, out_path=loss_curve_path)
...
if __name__ == "__main__":
    ...
    model = train(
        ...
        warmup_epochs=WARMUP_EPOCHS, class_names=CLASS_NAMES, loss_curve_path=LOSS_CURVE_PATH,
    )
