...
def get_transforms(augment: bool = False) -> transforms.Compose:
    ...
    - NO vertical flip  — time axis must stay upright.
    - NO rotation       — frequency/time structure is orientation-sensitive.
    - Light horizontal flip only during training (mirrors in time are OK for
      DRONE detection since the RF pattern is symmetric).
    ...
    """
    base = [
        transforms.Resize((224, 224)),      # exact spatial size
        transforms.ToTensor(),              # → [0.0, 1.0] float32, shape C×H×W
    ]
    ...
    if augment:
        ...
        # Insert augmentations BEFORE ToTensor (operates on PIL images)
        pipeline = [transforms.Resize((224, 224))] + augmentation + [transforms.ToTensor()]
    else:
        pipeline = base
