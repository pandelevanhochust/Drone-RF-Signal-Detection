...
def split_dataset(
    ...
    """
    Walk src_dir (DRONE/ and NO_DRONE/ sub-folders) and create:
        dest_dir/train/DRONE/    dest_dir/train/NO_DRONE/
        dest_dir/val/DRONE/      dest_dir/val/NO_DRONE/

    Parameters
    ----------
    src_dir     : Root folder with DRONE/ and NO_DRONE/ sub-directories.
    dest_dir    : Destination root; created if absent.
    split_ratio : Fraction reserved for validation (default 0.20).
    ...
    log.info("Classes found: %s", [d.name for d in class_dirs])
    ...
    for cls_dir in class_dirs:
        ...
        train_imgs, val_imgs = train_test_split(
            images, test_size=split_ratio, random_state=seed, shuffle=True
        )
        ...
        log.info("  %-12s → train: %d | val: %d",
                 cls_dir.name, len(train_imgs), len(val_imgs))

    log.info("Split complete → '%s'", dest)
...
def train(
    ...
    Loss: CrossEntropyLoss with label_smoothing=0.05 — reduces overconfidence
    on ambiguous borderline frames where the drone signal is faint.

    Scheduler: CosineAnnealingLR — decays LR smoothly from lr to lr×0.01
    over num_epochs, avoiding sharp drops that destabilise BN statistics.
    """
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        log.info("Epoch %2d/%d | train_loss=%.4f | val_loss=%.4f | val_acc=%.2f%%",
                 epoch, num_epochs, train_loss, val_loss, val_acc * 100)
...
if __name__ == "__main__":
    ...
    # Where the 80/20 train/val split will be written
